...
class Node(pygame.sprite.Sprite):
    def __init__(self, pos, status, icon_speed, path):
        ...
        if status == 'available':
            ...
            self.status = 'locked'
        self.rect = self.image.get_rect(center=pos) if self.image else pygame.Rect(0, 0, 0, 0)
    ...
    def animate(self):
        ...
        self.image = self.frames[int(self.frame_index)] if self.frames else None
...
class Icon(pygame.sprite.Sprite):
    def __init__(self, pos):
        super().__init__()
        self.pos = pos
        self.image = pygame.image.load('../grafika/poziomy/motul2.png').convert_alpha()
